...
describe('parseClientMessage', () => {
  it('accepts well formed messages and keeps seq', () => {
    ...
    expect(parse({ type: 'changeChip', v: 3, value: 50, into: [{ value: 1, count: 20 }, { value: 5, count: 6 }] })).toEqual({
      type: 'changeChip', v: 3, value: 50, into: [{ value: 1, count: 20 }, { value: 5, count: 6 }],
    });
    ...
    expect(parse({ type: 'confirmAward', v: 15 })).toEqual({ type: 'confirmAward', v: 15 });
    expect(parse({ type: 'disputeAward', v: 16 })).toEqual({ type: 'disputeAward', v: 16 });
    expect(parse({ type: 'proposeOverride', v: 17, reason: 'Table ruling', allocations: { abcdef01: 30 } })).toEqual({
  ...
  it('rejects malformed input', () => {
    const bad = [
      ...
      { type: 'changeChip', v: 1, value: 50 },
      { type: 'changeChip', v: 1, value: 50, into: [] },
